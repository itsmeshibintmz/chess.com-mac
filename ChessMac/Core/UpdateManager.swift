...
final class UpdateManager: ObservableObject {
    ...
    private let repoPath = "itsmeshibintmz/chess.com-mac"
    ...
    func checkForUpdates(manual: Bool = false) {
        ...
        request.setValue("ChessMacWrapper-Updater", forHTTPHeaderField: "User-Agent")
    ...
    func installUpdate(dmgPath: String) {
        let mountPath = "/tmp/chess_update_mount"
        ...
        let mountedAppPath = "\(mountPath)/Chess.com.app"
        guard fileManager.fileExists(atPath: mountedAppPath) else {
            print("Error: Chess.com.app bundle not found in mounted DMG.")
            return
        }
        ...
        exec > /tmp/chess_updater.log 2>&1
...
private final class DownloadProgressDelegate: NSObject, URLSessionDownloadDelegate {
    ...
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        let fileManager = FileManager.default
        let destinationURL = URL(fileURLWithPath: "/tmp/ChessUpdate.dmg")
